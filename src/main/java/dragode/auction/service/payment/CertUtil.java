package dragode.auction.service.payment;
...
public class CertUtil {
    ...
    /** 证书使用模式(单证书/多证书) */
    private static final Boolean SINGLE_MODE = Constant.SINGLE_MODE;

    private static final String SIGN_CER_PATH = Constant.CER_SIGN_PATH;
    private static final String SIGN_CER_PWD = Constant.CER_SIGN_PWD;
    private static final String SIGN_CER_TYPE = Constant.CER_SIGN_TYPE;
...
//	private static X509Certificate encryptTrackCert = null;
    ...
    public static void initSignCert() {
        ...
        try {
            keyStore = getKeyInfo(SIGN_CER_PATH, SIGN_CER_PWD, SIGN_CER_TYPE);
            logger.debug("InitSignCert Successful. CertId=[" + getSignCertId() + "]");
        } catch (IOException e) {
    ...
    public static void initSignCert(String certFilePath, String certPwd) {
        logger.debug("加载证书文件[" + certFilePath + "]和证书密码[" + certPwd + "]的签名证书开始.");
        ...
        logger.debug("加载证书文件[" + certFilePath + "]和证书密码[" + certPwd + "]的签名证书结束.");
    }
    ...
    public static void loadRsaCert(String certFilePath, String certPwd) {
        ...
        try {
            ...
            logger.debug("LoadRsaCert Successful");
        } catch (IOException e) {
    ...
    private static void initEncryptCert() {
        logger.debug("加载敏感信息加密证书==>" + Constant.CER_ENCRYPT_PATH);
        if (!isEmpty(Constant.CER_ENCRYPT_PATH)) {
            encryptCert = initCert(Constant.CER_ENCRYPT_PATH);
            logger.debug("LoadEncryptCert Successful");
        } else {
            logger.debug("WARN: acpsdk.encryptCert.path is empty");
        }
//		if (!isEmpty(Constants.getConfig().getEncryptTrackCertPath())) {
//			encryptTrackCert = initCert(Constants.getConfig()
//					.getEncryptTrackCertPath());
//			logger.debug("LoadEncryptTrackCert Successful");
//		} else {
//			logger.debug("WARN: acpsdk.encryptTrackCert.path is empty");
//		}
    }
    ...
    private static void initTrackKey() {
        if (!isEmpty(Constant.CER_ENCRYPT_TRACK_KEY_MODULUS)
                && !isEmpty(Constant.CER_ENCRYPT_TRACK_KEY_EXPONENT)) {
            encryptTrackKey = SecureUtil.getPublicKey(Constant.CER_ENCRYPT_TRACK_KEY_MODULUS,
                    Constant.CER_ENCRYPT_TRACK_KEY_EXPONENT);
            logger.debug("LoadEncryptTrackKey Successful");
        } else {
            logger.debug("WARN: acpsdk.encryptTrackKey.modulus or acpsdk.encryptTrackKey.exponent is empty");
        }
    }
    ...
    private static void initValidateCertFromDir() {
        certMap.clear();
        String dir = Constant.CER_VALIDATE_DIR;
        logger.debug("加载验证签名证书目录==>" + dir);
        if (isEmpty(dir)) {
            logger.debug("ERROR: acpsdk.validateCert.dir is empty");
            return;
        }
        CertificateFactory cf = null;
        FileInputStream in = null;
        try {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                certMap.put(validateCert.getSerialNumber().toString(),
                        validateCert);
                // 打印证书加载信息,供测试阶段调试
                logger.debug("[" + file.getAbsolutePath() + "][CertId="
                        + validateCert.getSerialNumber().toString() + "]");
            }
            logger.debug("LoadVerifyCert Successful");
        } catch (CertificateException e) {
    ...
    public static PublicKey getEncryptCertPublicKey() {
        if (null == encryptCert) {
            String path = Constant.CER_ENCRYPT_PATH;
            if (!isEmpty(path)) {
                encryptCert = initCert(path);
                return encryptCert.getPublicKey();
            } else {
                logger.debug("ERROR: acpsdk.encryptCert.path is empty");
                return null;
            }
        } else {
            return encryptCert.getPublicKey();
...
//			String path = Constants.getConfig().getEncryptTrackCertPath();
//			if (!isEmpty(path)) {
//				encryptTrackCert = initCert(path);
//				return encryptTrackCert.getPublicKey();
//			} else {
//				logger.debug("ERROR: acpsdk.encryptTrackCert.path is empty");
//				return null;
//			}
//		} else {
//			return encryptTrackCert.getPublicKey();
...
//		}
    ...
    public static KeyStore getKeyInfo(String pfxkeyfile, String keypwd, String type) throws IOException {
        logger.debug("加载签名证书==>" + pfxkeyfile);
        FileInputStream fis = null;
        try {
            KeyStore ks = KeyStore.getInstance(type, "BC");
            logger.debug("Load RSA CertPath=[" + pfxkeyfile + "],Pwd=[" + keypwd + "],type=[" + type + "]");
            ...
            if (Security.getProvider("BC") == null) {
                logger.debug("BC Provider not installed.");
            }
    ...
    public static void printSysInfo() {
        logger.debug("================= SYS INFO begin====================");
        logger.debug("os_name:" + System.getProperty("os.name"));
        logger.debug("os_arch:" + System.getProperty("os.arch"));
        logger.debug("os_version:" + System.getProperty("os.version"));
        logger.debug("java_vm_specification_version:"
                + System.getProperty("java.vm.specification.version"));
        logger.debug("java_vm_specification_vendor:"
                + System.getProperty("java.vm.specification.vendor"));
        logger.debug("java_vm_specification_name:"
                + System.getProperty("java.vm.specification.name"));
        logger.debug("java_vm_version:"
                + System.getProperty("java.vm.version"));
        logger.debug("java_vm_name:" + System.getProperty("java.vm.name"));
        logger.debug("java.version:" + System.getProperty("java.version"));
        logger.debug("java.vm.vendor=[" + System.getProperty("java.vm.vendor") + "]");
        logger.debug("java.version=[" + System.getProperty("java.version") + "]");

        printProviders();
        logger.debug("================= SYS INFO end=====================");
    }

    public static void printProviders() {
        logger.debug("Providers List:");
        Provider[] providers = Security.getProviders();
        for (int i = 0; i < providers.length; i++) {
            logger.debug(i + 1 + "." + providers[i].getName());
        }
    }
