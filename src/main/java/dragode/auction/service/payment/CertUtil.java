...
import static dragode.auction.service.payment.SignUtil.isEmpty;
...
//	private static X509Certificate encryptTrackCert = null;
	...
	public static void init() {
		addProvider();
		if (Constants.TRUE_STRING.equals(Constants.singleMode)) {
			// 单证书模式,初始化配置文件中的签名证书
			initSignCert();
		}
	...
	public static void initSignCert() {
		...
		try {
			keyStore = getKeyInfo(Constants.SignCertPath,
					Constants.SignCertPwd, Constants.SignCertType);
			logger.debug("InitSignCert Successful. CertId=["
					+ getSignCertId() + "]");
		} catch (IOException e) {
	...
	public static void initSignCert(String certFilePath, String certPwd) {
		logger.debug("加载证书文件[" + certFilePath + "]和证书密码[" + certPwd
				+ "]的签名证书开始.");
		certKeyStoreLocal.remove();
		File files = new File(certFilePath);
		if (!files.exists()) {
			logger.debug("证书文件不存在,初始化签名证书失败.");
			return;
		}
		try {
	...
	private static void initEncryptCert() {
		logger.debug("加载敏感信息加密证书==>"+Constants.EncryptCertPath);
		if (!isEmpty(Constants.EncryptCertPath)) {
			encryptCert = initCert(Constants.EncryptCertPath);
...
//		}
	...
	private static void initTrackKey() {
		if (!isEmpty(Constants.EncryptTrackKeyModulus)
				&& !isEmpty(Constants.EncryptTrackKeyExponent)) {
			encryptTrackKey = SecureUtil.getPublicKey(Constants.EncryptTrackKeyModulus,
					Constants.EncryptTrackKeyExponent);
	...
	private static void initValidateCertFromDir() {
		certMap.clear();
		String dir = Constants.ValidateCertDir;
	...
	public static PrivateKey getSignCertPrivateKey() {
		try {
			...
			PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias,
					Constants.SignCertPwd.toCharArray());
			return privateKey;
	...
	public static PublicKey getEncryptCertPublicKey() {
		if (null == encryptCert) {
			String path = Constants.EncryptCertPath;
			if (!isEmpty(path)) {
