...
public class SignUtil {
	...
	public static boolean sign(Map<String, String> data, String encoding) {
		...
		// 设置签名证书序列号
		data.put(Constants.param_certId, CertUtil.getSignCertId());
		// 将Map信息转换成key1=value1&key2=value2的形式
		String stringData = coverMap2String(data);
		logger.debug("待签名请求报文串:[" + stringData + "]");
		/**
		 * 签名\base64编码
		 */
		byte[] byteSign = null;
	...
	public static boolean signByCertInfo(Map<String, String> data,
		...
		if (isEmpty(certPath) || isEmpty(certPwd)) {
			logger.debug("Invalid Parameter:CertPath=[" + certPath
					+ "],CertPwd=[" + certPwd + "]");
			return false;
		}
		// 设置签名证书序列号
		data.put(Constants.param_certId,
	...
	public static Map<String, String> filterBlank(Map<String, String> contentData){
		...
		for(String key:keyset){
			...
			if (StringUtils.isNotBlank(value)) {
				...
				logger.debug(key + "-->" + String.valueOf(value));
			}
		}
